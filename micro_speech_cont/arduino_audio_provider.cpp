...
TfLiteStatus GetAudioSamples(tflite::ErrorReporter* error_reporter,
                             ...
                             int16_t* min_sample, int16_t* max_sample) {
  ...
  for (int i = 0; i < duration_sample_count; ++i) {
    ...
    if (*min_sample > g_audio_output_buffer[i]) {
      *min_sample = g_audio_output_buffer[i];
    } else if (*max_sample < g_audio_output_buffer[i]) {
      *max_sample = g_audio_output_buffer[i];
    }
  }
