...
TfLiteStatus FeatureProvider::PopulateFeatureData(
  ...
  if (slices_needed > kFeatureSliceCount || *how_many_new_slices < 0) {
    slices_needed = kFeatureSliceCount;
  }
  *how_many_new_slices = slices_needed;
  ...
  if (slices_needed > 0) {
    ...
    for (int new_slice = slices_to_keep; new_slice < kFeatureSliceCount;
      ...
      GetAudioSamples(error_reporter, (slice_start_ms > 0 ? slice_start_ms : 0),
                      ...
                      &audio_samples, &min_sample, &max_sample);
    ...
    *audio_threshold = *audio_threshold || (min_sample < -200 || max_sample > 200);

    #ifdef DEBUG_FEATURE
    
    TF_LITE_REPORT_ERROR(error_reporter,
                       "Thresholds %d %d %d",
                       min_sample, max_sample, *audio_threshold);
    ...
    for (int i = 0; i < kFeatureElementCount; i++) {
      feature_data_[i] = (int8_t)( g_audio_yes__1_data[i] - 128);
    }
    ...
    for (int i = 0; i < kFeatureElementCount; i++) {
      feature_data_[i] = (int8_t)( g_audio_no__1_data[i] - 128);
    }
  ...
  int8_t min_feature = 0;
  int8_t max_feature = 0;
  
  for (int i = 0; i < kFeatureElementCount; i++) {
    if (i == 0 || min_feature > feature_data_[i]) {
      min_feature = feature_data_[i];
    }
    if (i == 0 || max_feature < feature_data_[i]) {
      max_feature = feature_data_[i];
    }
  }
