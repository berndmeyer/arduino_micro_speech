...
volatile int32_t g_noise_heard_timestamp = 0;
int16_t g_noise_threshold = 1500;
}
...
bool DetectNoise(tflite::ErrorReporter* error_reporter) {
  ...
  if (!g_is_recording) {
    for (int i = 0; i < DEFAULT_PDM_BUFFER_SIZE / 2; i++){ 
      int16_t value = g_audio_buffer[i];
      if (value > g_noise_threshold || value < -1 * g_noise_threshold) {
        TF_LITE_REPORT_ERROR(error_reporter, "Heard noise %d @ %d", value, i);
        heard_noise = true;
        break;
      }
    }
